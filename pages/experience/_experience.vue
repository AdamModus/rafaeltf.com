  ...
  <main>
    <section v-if="post">
      ...
      <article class="experience-article">
        <img v-if="post.cover" class="cover-image" :src="post.cover" alt="" />
        ...
        <div v-if="post.gallery" class="nuxt-content">
          <img v-for="image in post.gallery" class="image" :key="image.id" :src="image" alt="" />
        </div>
      </article>
    </section>
  </main>
...
export default {
  async asyncData({ $content, params, error }) {
    let post;
    try {
      post = await $content('experience', params.experience).fetch();
    } catch (e) {
